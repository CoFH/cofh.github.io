...
for file in glob.glob("crafting.yml"):
  new_shapeless = {}
  new_shaped = {}
  with open(file,'r') as f:
    ...
    for recipe_id in data:
      if shapeless := data[recipe_id]["data"].pop("shapeless", False):
        new_data = new_shapeless
      else:
        new_data = new_shaped
      
      new_data[recipe_id] = {"inputs": {"item": []}, "outputs": {"item": [{}]}}
      
      if count := data[recipe_id]["data"]["result"].pop("amount", None):
        new_data[recipe_id]["outputs"]["item"][0]["count"] = count
      if ids := data[recipe_id]["data"]["result"].pop("item", None):
        new_data[recipe_id]["outputs"]["item"][0]["id"] = ids 
      else:
        new_data[recipe_id]["outputs"]["item"][0]["id"] = groups[data[recipe_id]["data"]["result"]["item-group"]]["items"]
      
      for row in data[recipe_id]["data"]["rows"]:
        for col in range(3):
          if col < len(row) and row[col]:
            new_slot = {}
            if count := row[col].pop("amount", None):
              new_slot["count"] = count
            if ids := row[col].pop("item", None):
              new_slot["id"] = ids
            else:
              new_slot["id"] = groups[row[col]["item-group"]]["items"]
            new_data[recipe_id]["inputs"]["item"].append(new_slot)
          elif not shapeless:
            new_slot = None
            new_data[recipe_id]["inputs"]["item"].append(new_slot)
  
  with open('output/crafting_shapeless.yml','w') as output:
    yaml.dump(new_shapeless, output, Dumper=NoAliasDumper)
  with open('output/crafting_shaped.yml','w') as output:
    yaml.dump(new_shaped, output, Dumper=NoAliasDumper)